...
export default function Login() {
  ...
  useEffect(() => {
    if (loginInput.message !== '') {
      const timer = setTimeout(() => {
        setLogin(prevState => ({ ...prevState, message: '' }))
      }, 4000)
  ...
  const loginSubmit = (e) => {
    ...
    apiaxios.get('/sanctum/csrf-cookie').then(response => {
      apiaxios.post(`api/login`, data).then(res => {
                if(res.data.status === 200){
                    localStorage.setItem('auth_token', res.data.token)
                    localStorage.setItem('auth_name', res.data.username)
                    localStorage.setItem('permissions', JSON.stringify(res.data.permissions))
                    history.push('/Amply-App/Home');
                }
                if (res.data.status === 401)
                {
                    ...
                    setLogin({...loginInput, message:res.data.message, error_list:  [],})
                ...
                {   
                    ...
                    setLogin({...loginInput, error_list:  res.data.validation_erros, message:'' })
  ...
  return (
    <>
        <Head>
          <title>Login Amply - Amplificando Possibilidades - Sistema e Métricas.</title>
      ...
      <main className={styles.MainLogin}>
        <div className={styles.CotainerLogin}>
            <div className={styles.FormHere}>
                <h1 className={styles.welcomeAmply}>Bem-vindo a</h1>
                ...
                <form onSubmit={loginSubmit}>
                  <InputEmailLabel onChange={handleInput} name="email" value={loginInput.email}/>
                  <br />
                  <InputPasswordLabel onChange={handleInput} name="password" value={loginInput.password} />
                  <br /><br />
                  {loginInput.message ? <div className={styles.retornoMessage}><p>{loginInput.error_list.email}</p> <p>{loginInput.message}</p> <p>{loginInput.error_list.password}</p></div> : ''}
                  ...
                  <div className={styles.submitArea}>
                      <div className={styles.rememberArea}>                                   
                          ...
                          <Link href="/Nova-Senha" className={styles.linkHelp}><h2>Esqueceu a senha ?</h2></Link>
                          </div>
                                
                          <InputSubmited value="LOGIN"/>
                  </div>
                </form>
            </div>
